...
impl CanvasImporter {
    ...
    pub async fn import_courses(&self) -> Result<(usize, usize, Vec<String>), String> {
        ...
        for canvas_course in canvas_courses {
            match self.import_single_course(&canvas_course).await {
                Ok(true) => imported += 1,
                Ok(false) => updated += 1,
                Err(e) => errors.push(format!("Error importing course {}: {}", canvas_course.id, e)),
            }
        }
        
        info!("Canvas course import completed: {} imported, {} updated, {} errors", 
    ...
    async fn import_single_course(&self, canvas_course: &CanvasCourse) -> Result<bool, String> {
        ...
        if let Some(row) = existing {
            ...
            task::spawn(self.import_course_enrollments(id.clone(), canvas_course.id));
    ...
    async fn import_course_enrollments(&self, course_id: String, canvas_course_id: i64) -> Result<(), String> {
        ...
        for enrollment in enrollments {
            // Import the user if they don't exist
            let user_id = self.ensure_user_exists(&enrollment.user).await?;
