...
echo "Starting Ordo Quiz Module Test..."

# Create test directories
mkdir -p ordo_quiz_test_data
mkdir -p migrations

# Create migration file
cat > migrations/20240421_ordo_quiz_schema.sql << 'EOF'
...
echo "Created migration file: migrations/20240421_ordo_quiz_schema.sql"
...
sqlite3 "$DB_PATH" ".read migrations/20240421_ordo_quiz_schema.sql"
if [ $? -ne 0 ]; then
    echo "Failed to apply migration"
    exit 1
fi

echo "Applied migration to database: $DB_PATH"

# Insert test data using sqlite3
...
sqlite3 "$DB_PATH" ".read test_data.sql"
if [ $? -ne 0 ]; then
    echo "Failed to insert test data"
    exit 1
fi

echo "Inserted test data into database: $DB_PATH"
...
# Check quizzes
echo -e "\nQuizzes:"
sqlite3 "$DB_PATH" "SELECT id, title FROM quizzes;"

# Check questions
echo -e "\nQuestions:"
sqlite3 "$DB_PATH" "SELECT id, question_text FROM questions;"

# Check quiz attempts
echo -e "\nQuiz Attempts:"
sqlite3 "$DB_PATH" "SELECT id, quiz_id, user_id, status, score FROM quiz_attempts;"

# Check activities
echo -e "\nActivities:"
sqlite3 "$DB_PATH" "SELECT id, user_id, quiz_id, activity_type FROM quiz_activities;"
