...
"#;
    ...
    let test_data_path = "test_data.sql";
    fs::write(test_data_path, test_data)?;
    
    let status = Command::new("sqlite3")
        .arg(db_path)
        .arg(".read test_data.sql")
        .status()?;
    ...
    let output = Command::new("sqlite3")
        .arg(db_path)
        .arg("SELECT id, title FROM quizzes;")
        .output()?;
    ...
    let output = Command::new("sqlite3")
        .arg(db_path)
        .arg("SELECT id, question_text FROM questions;")
        .output()?;
    ...
    let output = Command::new("sqlite3")
        .arg(db_path)
        .arg("SELECT id, quiz_id, user_id, status, score FROM quiz_attempts;")
        .output()?;
    ...
    let output = Command::new("sqlite3")
        .arg(db_path)
        .arg("SELECT id, user_id, quiz_id, activity_type FROM quiz_activities;")
        .output()?;
