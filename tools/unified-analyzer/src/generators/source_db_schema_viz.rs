...
use crate::analyzers::source_db_schema_analyzer::SourceDbSchemaAnalyzer;

/// Generate a database schema visualization from source code
pub fn generate_source_db_schema_visualization(canvas_path: &str, discourse_path: &str, output_dir: &PathBuf) -> Result<()> {
    ...
    let mut analyzer = SourceDbSchemaAnalyzer::new(canvas_path, discourse_path);
    analyzer.analyze()?;
