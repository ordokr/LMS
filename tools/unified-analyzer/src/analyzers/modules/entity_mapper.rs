...
impl EntityMapper {
    ...
    /// Create a new EntityMapper with caching disabled
    ...
    pub fn extract_canvas_entities(&mut self, canvas_path: &Path) -> Result<()> {
        ...
        // First collect all Ruby model files
        let mut model_files = Vec::new();
        self.walk_directory(&models_dir, |file_path| {
            if let Some(ext) = file_path.extension() {
                if ext == "rb" {
                    model_files.push(file_path.to_path_buf());
                }
            }
        })?;
        ...
        for file_path in model_files {
            ...
            let should_parse = if self.use_cache {
                match fs::metadata(&file_path) {
                            ...
                            if let Ok(modified_secs) = modified_time.duration_since(SystemTime::UNIX_EPOCH) {
                                ...
                                if let Some(cache) = self.file_cache.get(&file_path_str) {
                                    if cache.last_modified >= modified_secs {
                                        // File hasn't been modified, use cached entity
                                        entities.push(cache.entity.clone());
                                        println!("Using cached Canvas entity: {}", cache.entity.entity);
                                        false
                                    } else {
                                        // File has been modified, re-parse
                                        true
                                    }
                                } else {
                                ...
                                // Couldn't get modified time, parse it
                                true
                            }
                        } else {
                            // Couldn't get modified time, parse it
                    ...
                    Err(_) => {
                        // Couldn't get metadata, parse it
                        true
                    }
                }
            } else {
                // Caching disabled, always parse
                true
            };
                            ...
                            if let Ok(modified_secs) = modified_time.duration_since(SystemTime::UNIX_EPOCH) {
                                ...
                                if let Some(cache) = self.file_cache.get(&file_path_str) {
                                    if cache.last_modified >= modified_secs {
                                        // File hasn't been modified, use cached entity
                                        entities.push(cache.entity.clone());
                                        println!("Using cached Discourse entity: {}", cache.entity.entity);
                                        false
                                    } else {
                                        // File has been modified, re-parse
                                        true
                                ...
                                if let Ok(modified_secs) = modified_time.duration_since(SystemTime::UNIX_EPOCH) {
                                    ...
                                    if let Some(cache) = self.file_cache.get(&file_path_str) {
                                        if cache.last_modified >= modified_secs {
                                            // File hasn't been modified, use cached entity
                                            entities.push(cache.entity.clone());
                                            println!("Using cached Ordo entity: {}", cache.entity.entity);
                                            false
                                        } else {
                                            // File has been modified, re-parse
                                            true
    ...
    pub fn get_entities_by_category(&self, category: &str) -> Vec<String> {
        let mut result = Vec::new();

        for (source, entities) in &self.entities {
            for entity in entities {
                if entity.category == category {
                    result.push(format!("{}.{}", source, entity.entity));
                }
            }
        }
    ...
    fn parse_ruby_model(&self, content: &str, file_path: &Path, source: &str) -> Option<NormalizedEntity> {
        ...
        let schema_regex = Regex::new(r"create_table\s+(\w+)").ok()?;
        for cap in schema_regex.captures_iter(content) {
            if let Some(field_name) = cap.get(1) {
                fields.insert(field_name.as_str().to_string(), "table".to_string());
            }
        }
        ...
        // Determine category based on name and fields
        let category = self.determine_entity_category(class_name, &fields);
    ...
    fn parse_rust_model(&self, content: &str, file_path: &Path, source: &str) -> Option<NormalizedEntity> {
        ...
        // Determine category based on name and fields
        let category = self.determine_entity_category(struct_name, &fields);
    ...
    fn map_entity(&self, source_entity: &NormalizedEntity, target_entities: &[NormalizedEntity]) -> Option<EntityMapping> {
        ...
        if let Some(target) = exact_match {
            ...
            for (source_field, _source_type) in &source_entity.fields {
                if let Some(_target_type) = target.fields.get(source_field) {
    ...
    /// Determine the category of an entity based on its name and fields
    fn determine_entity_category(&self, name: &str, fields: &HashMap<String, String>) -> String {
        let name_lower = name.to_lowercase();

        // Check for common entity categories based on name
        if name_lower.contains("user") || name_lower.contains("account") || name_lower.contains("profile") {
            return "user".to_string();
        }

        if name_lower.contains("course") || name_lower.contains("class") || name_lower.contains("section") {
            return "course".to_string();
        }

        if name_lower.contains("assignment") || name_lower.contains("submission") || name_lower.contains("quiz") {
            return "assignment".to_string();
        }

        if name_lower.contains("discussion") || name_lower.contains("topic") || name_lower.contains("post") || name_lower.contains("comment") {
            return "discussion".to_string();
        }

        if name_lower.contains("grade") || name_lower.contains("score") {
            return "grading".to_string();
        }

        if name_lower.contains("file") || name_lower.contains("attachment") || name_lower.contains("document") {
            return "file".to_string();
        }

        if name_lower.contains("notification") || name_lower.contains("message") || name_lower.contains("alert") {
            return "notification".to_string();
        }

        // Check fields for clues about category
        if fields.contains_key("user_id") || fields.contains_key("account_id") {
            if fields.contains_key("course_id") || fields.contains_key("section_id") {
                return "enrollment".to_string();
            }
        }
    ...
    fn names_are_similar(&self, name1: &str, name2: &str) -> bool {
        ...
        if name1_lower.clone() + "s" == name2_lower || name2_lower.clone() + "s" == name1_lower {
            return true;
        }
