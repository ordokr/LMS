#!/usr/bin/env python
"""
Create a Canvas analyzer template for the unified analyzer.
"""

import os
import sys

def create_canvas_analyzer_template(canvas_rust_path):
    """Create a Canvas analyzer template based on the JavaScript analyzer"""
...
impl CanvasAnalyzer {
                ...
                if let Some(file_name) = controller_file.file_name() {
                    ...
                    if let Some(controller_name) = file_name.strip_suffix("_controller.rb") {
                        ...
                        self.result.controllers.push(CanvasController {
                            ...
                            file_name: controller_name.to_string(),
    ...
    fn analyze_dependencies(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        if gemfile_path.exists() {
            if let Ok(content) = fs::read_to_string(&gemfile_path) {
                let gem_regex = Regex::new(r"^\s*gem\s+(['\"])(\w+)\1").unwrap();
                
                for captures in gem_regex.captures_iter(&content) {
                    if let Some(gem_name) = captures.get(2) {
    ...
    parent_dir = os.path.dirname(canvas_rust_path)
    os.makedirs(parent_dir, exist_ok=True)
    
    # Write the content to the file
    with open(canvas_rust_path, 'w', encoding='utf-8') as f:
        f.write(content)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python canvas_analyzer_template.py <output_path>")
        sys.exit(1)
    
    output_path = sys.argv[1]
    create_canvas_analyzer_template(output_path)
    print(f"Created Canvas analyzer template at {output_path}")
